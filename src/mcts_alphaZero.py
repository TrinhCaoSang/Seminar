...
def save_for_lime(state, visit_counts, move_probs):
    lime_data.append({
        "state": state.tolist(),
        "visit": visit_counts,
        "probs": move_probs
...
class MCTS(object):
    ...
    def get_move_probs(self, state, temp=1e-3):
        ...
        # 🎯 Lưu dữ liệu MCTS ra file JSON
        data = {
            "state": state.current_state().tolist(),  # Trạng thái bàn cờ (3 kênh: quân của mình, quân đối thủ, trạng thái đặc biệt).
            "actions": list(acts),  # Các nước đi khả thi
            "visit_counts": list(visits),  # Số lần thăm của MCTS
            "probs": list(act_probs),  # Xác suất chính sách
        }

        with open("mcts_data.json", "a") as f:
            json.dump(data, f)
            f.write("\n")
